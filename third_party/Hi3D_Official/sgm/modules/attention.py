...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        self, x, context=None, additional_tokens=None, n_times_crossframe_attn_in_self=0, context_instead_of_self_attn=None,
        use_reentrant=None
    ):
        ...
        if context_instead_of_self_attn is not None:
            kwargs.update({"context_instead_of_self_attn": context_instead_of_self_attn})
    ...
    def _forward(
        self, x, context=None, context_instead_of_self_attn=None, additional_tokens=None, n_times_crossframe_attn_in_self=0
    ):
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None, inpainting_mask=None):
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            x = block(x, context=context[i], inpainting_mask=inpainting_mask)
        if self.use_linear:
            x = self.proj_out(x)
